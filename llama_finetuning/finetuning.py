...
from .config import LlamaFineTuningConfig


class LlamaFineTuning:
    """Main class for Llama fine-tuning with LoRA and optimized hyperparameters."""

    def __init__(self, config: LlamaFineTuningConfig):
        self.config = config
        self.model = None
        self.tokenizer = None
        self.trainer = None
    ...
    @staticmethod
    def format_prompt(question: str, options: list, answer: str = None) -> str:
    ...
    def prepare_dataset(self, data: list) -> Dataset:
        ...
        for example in data:
            text = self.format_prompt(
                example["question"], example["options"], example["answer"]
            )
    ...
    def setup_model(self):
        """Set up model and tokeniser with optimised configuration."""
        print("Loading model and tokeniser...")

        # Load model without quantisation (following Qwen3 pattern)
        ...
        # LoRA configuration (updated with proven parameters)
    ...
    def setup_trainer(self, train_data: list):
        """Set up trainer with optimised configuration."""
        print("Setting up trainer...")

        train_dataset = self.prepare_dataset(train_data)

        # Training arguments (optimised based on research)
        ...
        print(
            f"Training steps: {len(train_dataset) // self.config.effective_batch_size * self.config.num_epochs}"
        )
    ...
    def evaluate_model(self, test_data: list) -> float:
        ...
        for example in tqdm(test_data, desc="Evaluating"):
            prompt = self.format_prompt(example["question"], example["options"])
        ...
        if failed_extractions > 0:
            print(f"⚠️  Failed to extract answer: {failed_extractions}/{total}")
        else:
